...
{
	static public T[] Deserialize<T>(string text, bool isColMarked = true)
	{
		return (T[])CreateArray(typeof(T), ParseCSV(text), isColMarked);
	}

	static public T[] Deserialize<T>(List<string[]> rows, bool isColMarked = true)
	{
		return (T[])CreateArray(typeof(T), rows, isColMarked);
	}
	...
	private static object Create(string[] cols, Type type)
	{
