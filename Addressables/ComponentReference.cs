...
namespace PM.UsefulThings.Memory
{
    [Serializable]
    public class ComponentReference<TComponent> : AssetReferenceT<TComponent> where TComponent : UnityEngine.Component
    {
            ...
            bool instantiateInWorldSpace = false)
        {
            return Addressables.ResourceManager.CreateChainOperation<TComponent, GameObject>(
                base.InstantiateAsync(parent, instantiateInWorldSpace), GameObjectReadyHandler);
        }

        public AsyncOperationHandle<TComponent> InstantiateComponentAsync(Vector3 position, Quaternion rotation,
            Transform parent = null)
        {
            return Addressables.ResourceManager.CreateChainOperation<TComponent, GameObject>(
                base.InstantiateAsync(position, rotation, parent), GameObjectReadyHandler);
        }
