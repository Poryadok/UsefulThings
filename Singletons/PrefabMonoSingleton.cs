...
{
	public class PrefabMonoSingleton<T> : MonoBehaviour where T : MonoBehaviour
	{
			...
			get
			{
				...
				lock (_lock)
				{
					if (_instance == null)
					{
						...
						if (_instance == null)
						{
							//instance not found. create new instance from prefab from resources
							var sample = Resources.LoadAll<T>("").First();

							if (sample == null)
							{
								Debug.LogError("[Singleton] Something went really wrong " +
									" - there is no prefab sample in resources!");
								return null;
							}

							_instance = Instantiate<T>(sample);
